...
import { 
  ...
  FaExclamationTriangle, FaTrashAlt, FaNotesMedical, FaDna
} from "react-icons/fa";
...
export default function SkinCancer() {
  ...
  const runAnalysis = () => {
    ...
    const sequence = [
      ...
      { t: 4800, msg: "Analysis Complete. Compiling diagnostic report...", type: "success" }
    ];
    ...
    setTimeout(() => {
      ...
      setResult({
        diagnosis: "Malignant Melanoma Detected", 
        confidence: 94.2,
        severity: "High Risk",
        notes: "High degree of asymmetry and border irregularity observed. Immediate biopsy and dermatological consultation strongly recommended."
      });
    }, 5500);
  };
            ...
            <AnimatePresence mode="wait">
              {result && (
                ...
                >
                  ...
                  <div className={`h-3 w-full ${result.severity.includes("High") ? "bg-red-500" : "bg-green-500"}`}></div>
                  
                  <div className="p-8">
                    <div className="flex justify-between items-start mb-6">
                      <div>
                        <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">CNN Prediction</p>
                        <h2 className={`text-2xl font-black leading-tight ${result.severity.includes("High") ? "text-red-600" : "text-green-600"}`}>
                          {result.diagnosis}
                        </h2>
                      </div>
                      <div className={`p-4 rounded-2xl shrink-0 ${result.severity.includes("High") ? "bg-red-50 text-red-600" : "bg-green-50 text-green-600"}`}>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      ...
                      <div className={`p-4 rounded-2xl border ${result.severity.includes("High") ? "bg-red-50 border-red-100" : "bg-green-50 border-green-100"}`}>
                        <p className="text-[10px] font-bold uppercase tracking-wider mb-1 opacity-70">Risk Level</p>
                        <p className={`text-xl font-black ${result.severity.includes("High") ? "text-red-700" : "text-green-700"}`}>
                          {result.severity}
                        </p>
                      </div>
                    </div>
